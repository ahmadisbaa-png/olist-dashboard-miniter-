...
with c1:
    ...
    fig = px.bar(seg_rev_df, x="rfm_segment", y="total_revenue",
...
with c2:
    ...
    fig2 = px.bar(seg_cnt_df, x="rfm_segment", y="customers",
...
c3, c4 = st.columns(2)
with c3:
    fig3 = px.bar(state_perf.head(10), x="customer_state", y="total_revenue",
                  title="Top 10 State by Revenue",
                  labels={"customer_state":"State", "total_revenue":"Revenue (R$)"})
    st.plotly_chart(fig3, use_container_width=True)

with c4:
    fig4 = px.bar(state_perf.sort_values("total_orders", ascending=False).head(10),
